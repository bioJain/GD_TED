...
- **사람에서의 TSHR ectodomain shedding rate는 문헌상 미보고**: 확인한 문헌은 TSHR의 constitutive intramolecular cleavage와 A-subunit shedding을 정성적으로 입증하지만, 정상인/GD 환자의 thyroid에서 단위 시간당 방출량, 전체 surface TSHR 중 방출 비율, 또는 in vivo half-life를 제시하지 않음[1-4]. 따라서 추정 백분율을 분자 스펙에 입력하지 않음.
...
- `문헌상 미보고`는 존재하지 않음을 증명한 것이 아니라, 아래 PubMed-indexed mechanism literature와 repo의 direct-TSHR program evidence를 확인한 범위에서 설계에 사용할 검증 수치를 발견하지 못했다는 의미임.
...
확인일: **2026-09-24**. PMID는 PubMed record identifier.
