...
> **Data cut:** repo frozen data는 2026-09-10, 아래 `P1`~`P7` PubMed 보완 검색은 2026-09-24 기준. 이 문서는 임상 처방 지침이 아니라, severity/activity/phenotype과 지역별 authorization/access를 분리한 경쟁환경 요약이다. `unresolved`는 공식 근거를 이번 source set에서 확정하지 못했다는 뜻이며, 비승인 또는 비급여를 뜻하지 않는다. 아래 `[N]`은 별도 표시가 없는 한 Biomni v2 참고문헌 번호다.
...
2026-09-24에 PubMed E-utilities로 `Graves ophthalmopathy methylprednisolone randomized`, `selenium Graves orbitopathy`, `teprotumumab reactivation durability thyroid eye disease`, `tocilizumab corticosteroid resistant Graves orbitopathy randomized`를 검색했다. 사람 대상 original trial/long-term follow-up을 우선하고 review는 IVGC 약 80% 종합치와 안전성 경계 확인에만 사용했다. 초록만 확인 가능한 논문은 초록에 제시된 수치만 사용했으며 full text의 미공개 분모를 역산하지 않았다.
